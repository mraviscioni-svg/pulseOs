  ...
  <div class="card lg:col-span-2">
    ...
    <div class="max-h-64 overflow-y-auto space-y-2 text-sm">
      <template x-for="p in filtered" :key="p.id">
        <button type="button" @click="addItem(p)" class="flex w-full justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-left transition hover:border-slate-300 hover:bg-slate-50">
      ...
      <tbody>
        <template x-for="(item, i) in items" :key="i">
          <tr class="border-t border-slate-100">
            ...
            <td><input type="number" step="0.001" x-model.number="item.quantity" class="input-field !w-20 !py-1"></td>
            <td><input type="number" step="0.01" x-model.number="item.unit_cost" class="input-field !w-24 !py-1"></td>
